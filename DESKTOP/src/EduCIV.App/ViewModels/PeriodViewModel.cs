...
public partial class PeriodViewModel : ObservableObject
{
    private readonly IAuthService _auth;
    private readonly IPeriodsApiService _periodsApi;
    ...
    public PeriodViewModel(IAuthService auth, IPeriodsApiService periodsApi)
    {
        _auth = auth;
        _periodsApi = periodsApi;
    }
    ...
    private async Task LoadItems()
    {
        ...
        try
        {
            ...
            var response = await _periodsApi.GetAll(schoolId);
    ...
    private async Task SaveItem()
    {
        ...
        try
        {
            var request = new CreatePeriodRequest
            {
                ...
                EndDate = DateTime.TryParse(GetProp(SelectedItem, "EndDate"), out var e) ? e : DateTime.Now.AddMonths(3),
            ...
            if (IsEditMode && GetProp(SelectedItem, "Id") is string id && !string.IsNullOrEmpty(id))
                response = await _periodsApi.Update(id, request);
            else
                response = await _periodsApi.Create(request);
    ...
    private async Task DeleteItem(object? item)
    {
        ...
        IsLoading = true;
        try { var r = await _periodsApi.Delete(id); if (r?.Success == true) await LoadItems(); }
        catch (Exception ex) { MessageBox.Show($"Erreur: {ex.Message}", "Erreur", MessageBoxButton.OK, MessageBoxImage.Error); }
        finally { IsLoading = false; }
    }
