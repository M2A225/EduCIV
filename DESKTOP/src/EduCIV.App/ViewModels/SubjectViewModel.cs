...
public partial class SubjectViewModel : ObservableObject
{
    ...
    private readonly ISubjectsApiService _subjectsApi;
    ...
    public SubjectViewModel(IAuthService auth, ISubjectsApiService subjectsApi)
    {
        _auth = auth;
    ...
    private async Task LoadItems()
    {
        ...
        try
        {
            ...
            var response = await _subjectsApi.GetAll(schoolId);
    ...
    private async Task SaveItem()
    {
        ...
        try
        {
            var request = new CreateSubjectRequest
            {
                ...
                Code = GetProp(SelectedItem, "Code"),
            ...
            if (IsEditMode && GetProp(SelectedItem, "Id") is string id && !string.IsNullOrEmpty(id))
                response = await _subjectsApi.Update(id, request);
            else
                response = await _subjectsApi.Create(request);
    ...
    private async Task DeleteItem(object? item)
    {
        ...
        IsLoading = true;
        try { var r = await _subjectsApi.Delete(id); if (r?.Success == true) await LoadItems(); }
        catch (Exception ex) { MessageBox.Show($"Erreur: {ex.Message}", "Erreur", MessageBoxButton.OK, MessageBoxImage.Error); }
        finally { IsLoading = false; }
    }
